...
class AuroraDns(common.BaseDns):
    ...
    def __init__(
        ...
        self.logger = get_logger(__name__).bind(dns_provider_name=self.dns_provider_name)
    ...
    def create_dns_record(self, domain_name, base64_of_acme_keyauthorization):
        ...
        self.delete_dns_record(domain_name=domain_name, base64_of_acme_keyauthorization=base64_of_acme_keyauthorization)
        ...
        record = zone.create_record(name=subDomain, type=RecordType.TXT, data=base64_of_acme_keyauthorization)
        return record
    ...
    def delete_dns_record(self, domain_name, base64_of_acme_keyauthorization):
        ...
        for x in records:
            if x.name==subDomain and x.type=='TXT':
                record_id = x.id
                self.logger.info('found' + subDomain + '.' + domainSuffix + 'with id : ' + record_id)

        try:
            record_id
        except NameError:
            self.logger.info('No record to delete. ' + subDomain + '.' + domainSuffix + ' not found.')
            return
